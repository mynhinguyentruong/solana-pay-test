...
export default async function apiHandler(req, res) {
  ...
  const keypair = Keypair.fromSecretKey(
    Uint8Array.from(JSON.parse(process.env.MERCHANT_PRIVATE_KEY))
  );
  ...
  const memo = "JC#4098";
  const link = new URL(
    "https://solana-send-sol-frontend-nhi-ymihn.vercel.app/api/checkout"
  );

  const testUrl = encodeURL({ link });
